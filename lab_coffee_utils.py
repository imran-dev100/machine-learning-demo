...
def plt_roast(X,Y):
    ...
    ax.scatter(X[Y==1,0],X[Y==1,1], s=70, marker='x', c='darkred', label="Good Roast" )
    ax.scatter(X[Y==0,0],X[Y==0,1], s=100, marker='o', facecolors='none', 
               edgecolors="darkblue",linewidth=1,  label="Bad Roast")
...
def plt_layer(X,Y,W1,b1,norm_l):
    ...
    for i in range(W1.shape[1]):
        ...
        ax[i].scatter(X[Y==1,0],X[Y==1,1], s=70, marker='x', c='darkred', label="Good Roast" )
        ax[i].scatter(X[Y==0,0],X[Y==0,1], s=100, marker='o', facecolors='none', 
                   edgecolors="darkblue",linewidth=1,  label="Bad Roast")
        tr = np.linspace(175,260,50)
        ax[i].plot(tr, (-3/85) * tr + 21, color="purple",linewidth=2)
        ax[i].axhline(y= 12, color="purple", linewidth=2)
        ax[i].axvline(x=175, color="purple", linewidth=2)
...
def plt_network(X,Y,netf):
    ...
    Y = Y.reshape(-1,)
    plt_prob(ax[0], netf)
    ax[0].scatter(X[Y==1,0],X[Y==1,1], s=70, marker='x', c='darkred', label="Good Roast" )
    ax[0].scatter(X[Y==0,0],X[Y==0,1], s=100, marker='o', facecolors='none', 
                   edgecolors=dlc["darkblue"],linewidth=1,  label="Bad Roast")
    ax[0].plot(X[:,0], (-3/85) * X[:,0] + 21, color="purple",linewidth=1)
    ...
    ax[1].plot(X[:,0], (-3/85) * X[:,0] + 21, color="purple",linewidth=1)
    ax[1].axhline(y= 12, color="purple", linewidth=1)
    ax[1].axvline(x=175, color="purple", linewidth=1)
    fwb = netf(X)
    yhat = (fwb > 0.5).astype(int)
    ax[1].scatter(X[yhat[:,0]==1,0],X[yhat[:,0]==1,1], s=70, marker='x', c='orange', label="Pdarkredicted Good Roast" )
    ax[1].scatter(X[yhat[:,0]==0,0],X[yhat[:,0]==0,1], s=100, marker='o', facecolors='none', 
                   edgecolors=dlc["darkblue"],linewidth=1,  label="Pdarkredicted Bad Roast")
